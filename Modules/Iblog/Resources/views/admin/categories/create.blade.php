...
@section('content-header')
   <h1>
      {{ trans('iblog::category.title.create category') }}
   </h1>
   <ol class="breadcrumb">
      <li><a href="{{ route('dashboard.index') }}"><i
                    class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
      <li><a href="{{ route('admin.iblog.category.index') }}">{{ trans('iblog::category.title.categories') }}</a></li>
      <li class="active">{{ trans('iblog::category.title.create category') }}</li>
   </ol>
@stop

@section('content')
   {!! Form::open(['route' => ['admin.iblog.category.store'], 'method' => 'post']) !!}
   <div class="row">
      <div class="col-xs-12 col-md-9">
         <div class="row">
                     ...
                     <div class="tab-content">
                        ...
                        @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                           ...
                           <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                              @include('iblog::admin.categories.partials.create-fields', ['lang' => $locale])
                           </div>
                        @endforeach
            ...
            @if (config('asgard.iblog.config.fields.category.partials.normal.create')&&config('asgard.iblog.config.fields.category.partials.normal.create') !== [])
               <div class="col-xs-12 ">
                  <div class="box box-primary">
                     <div class="box-header">
                     </div>
                     <div class="box-body ">
                        @foreach (config('asgard.iblog.config.fields.category.partials.normal.create') as $partial)
                           @include($partial)
                        @endforeach
                  ...
                  <div class="box-body ">
                     <div class="box-footer">
                        ...
                        <a class="btn btn-danger pull-right btn-flat"
                           href="{{ route('admin.iblog.category.index')}}"><i
